...
def convert_xml_to_label_image(xml_path, label_classes_dict, label_priority_order):
    ...
    color_multiplicator = 255//len(label_priority_order)
    color_multiplicator = 1
    for key in label_priority_order:
        for pts in objects_dict[key]:
            class_value = label_classes_dict[key]*color_multiplicator
...
def main(xml_directory, label_directory):
    ...
    for xml_path in xml_annotation_path_list:
        ...
        label_name = os.path.basename(xml_path)[:-4]
        # Save label_image
        label_image_path = os.path.join(dataset_labels_path, f"{label_name}.png")
